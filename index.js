...
ptz.ev.on("groups.update", async (anu) => {
			if (global.welcome) {
			try {
...
ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'
			...
			if (res.announce == true) {
				setTimeout(() => {
				ptz.sendMessage(res.id, {
					text: `*${X}[ S I S T E M ]${X}* group di senyapkan oleh admin`,
				})
				}, 2000)
			} else if (res.announce == false) {
				setTimeout(() => {
				ptz.sendMessage(res.id, {
					text: `*${X}[ S I S T E M ]${X}* group telah dibuka`,
				})
					}, 2000)
			} else if (res.restrict == true) {
				setTimeout(() => {
				ptz.sendMessage(res.id, {
					text: `*${X}[ S I S T E M ]${X}* suksess membatasi akses member`,
				})
					}, 2000)
				...
				setTimeout(() => {
				ptz.sendMessage(res.id, { 
					text: `*${X}[ S I S T E M ]${X}* deskripsi di ubah `+readmore+`\n\n${res.desc}`,
				})
					}, 2000)
      } else {
				setTimeout(() => {
				ptz.sendMessage(res.id, {
					text: `*${X}[ S I S T E M ]${X}* group telah di *${res.subject}*`,
				})
				}, 2000)
			} 
			}
...
console.log(comand)
ptz.appenTextMessage(comand, chatUpdate)
}
}
}
})
